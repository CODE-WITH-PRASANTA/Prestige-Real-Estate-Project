...
export default function Pricing() {
  ...
  useEffect(() => {
    const elements = sectionRef.current.querySelectorAll(".pricing-reveal");

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            entry.target.classList.add("pricing-active");
          }
        });
      },
      { threshold: 0.18 }
  ...
  return (
    <section className="pricing" ref={sectionRef}>
      <div className="pricingGlow pricingGlowOne"></div>
      <div className="pricingGlow pricingGlowTwo"></div>
      ...
      <div className="priceWrap">
        <div className="pricingTop">
          ...
          <div className="billingToggle pricing-reveal pricing-delay-1">
        ...
        <div
          className="priceSlider pricing-reveal pricing-delay-2"
          onMouseDown={onDown}
          onMouseUp={onUp}
          onTouchStart={onDown}
          onTouchEnd={onUp}
        >
          {isMobile && (
            <button className="priceNav left" onClick={prev} type="button">
              ‹
            </button>
          )}
          ...
          >
            {plans.map((p, index) => (
              <article
                key={p.id}
                className={`priceCard pricing-reveal pricing-card-delay-${index + 1}`}
              >
          ...
          {isMobile && (
            <button className="priceNav right" onClick={next} type="button">
              ›
            </button>
          )}
        </div>
        ...
        {isMobile && (
          <div className="priceDots pricing-reveal pricing-delay-3">
            {plans.map((_, i) => (
              <span
                key={i}
